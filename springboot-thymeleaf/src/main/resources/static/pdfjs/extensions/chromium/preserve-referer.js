 ...
 * pdfHandler.pdfjs will call saveReferer(details) when it is about to redirect to
 ...
 *
 * See setReferer in chromecom.pdfjs for more explanation of this logic.
 */
...
chrome.runtime.onConnect.addListener(function onReceivePort(port) {
  ...
  port.onMessage.addListener(function(data) {
    ...
    // know about the referer if stored in the history state (see chromecom.pdfjs).
    if (data.referer) {
  ...
  // Expose some response headers for fetch API calls from PDF.pdfjs;
  ...
  function exposeOnHeadersReceived(details) {
    ...
    // List of headers that PDF.pdfjs uses in src/display/network_utils.pdfjs
    var acehValue =
