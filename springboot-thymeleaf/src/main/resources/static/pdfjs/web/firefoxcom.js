...
let FirefoxCom = (function FirefoxComClosure() {
  return {
    ...
    requestSync(action, data) {
      ...
      sender.initCustomEvent('pdf.pdfjs.message', true, false,
    ...
    request(action, data, callback) {
      let request = document.createTextNode('');
      if (callback) {
        document.addEventListener('pdf.pdfjs.response', function listener(event) {
          let node = event.target;
          let response = event.detail.response;

          document.documentElement.removeChild(node);

          document.removeEventListener('pdf.pdfjs.response', listener);
...
// l10n.pdfjs for Firefox extension expects services to be set.
