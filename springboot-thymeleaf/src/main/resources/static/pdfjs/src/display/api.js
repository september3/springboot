...
if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('GENERIC')) {
  ...
  if (typeof window === 'undefined') {
    // node.pdfjs - disable worker and set require.ensure.
  ...
  if (typeof requirejs !== 'undefined' && requirejs.toUrl) {
    fallbackWorkerSrc = requirejs.toUrl('pdfjs-dist/build/pdf.worker.pdfjs');
  }
      ...
      __non_webpack_require__.ensure([], function() {
        try {
          ...
          if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('LIB')) {
            worker = __non_webpack_require__('../pdf.worker.pdfjs');
          } else {
            worker = __non_webpack_require__('./pdf.worker.pdfjs');
          }
 ...
 *   environments without DOM image and canvas support, such as e.g. Node.pdfjs.
 *   Valid values are 'decode', 'display' or 'none'; where 'decode' is intended
 *   for browsers with full image/canvas support, 'display' for environments
 *   with limited image support through stubs (useful for SVG conversion),
 *   and 'none' where JPEG images will be decoded entirely by PDF.pdfjs.
 ...
 *   for environments without `XMLHttpRequest` support, such as e.g. Node.pdfjs.
 ...
 *   data. By default PDF.pdfjs attempts to load PDFs in chunks.
 *   The default value is `false`.
 * @property {boolean} disableAutoFetch - (optional) Disable pre-fetching of PDF
 *   file data. When range requests are enabled PDF.pdfjs will automatically keep
 ...
 *   PDF.pdfjs (see `web/debugger.pdfjs`). The default value is `false`.
 */
 ...
 * @property {string} fontName - font name used by pdf.pdfjs for converted font.
 */
  ...
  postMessage(obj, transfers) {
    function cloneValue(value) {
      ...
      if ((buffer = value.buffer) && isArrayBuffer(buffer)) {
        ...
        if (value === buffer) {
          // Special case when we are faking typed arrays in compatibility.pdfjs.
          result = value;
...
/**
 * PDF.pdfjs web worker abstraction, it controls instantiation of PDF documents and
...
const PDFWorker = (function PDFWorkerClosure() {
  ...
  function setupFakeWorkerGlobal() {
    ...
    // In the developer build load worker_loader.pdfjs which in turn loads all the
    // other files and resolves the promise. In production only the
    // pdf.worker.pdfjs file is needed.
  ...
  class PDFWorker {
    ...
    _initialize() {
      ...
      // all requirements to run parts of pdf.pdfjs in a web worker.
  ...
  setupMessageHandler() {
        ...
        case 'JpegStream':
          return new Promise((resolve, reject) => {
            ...
            img.onerror = function() {
              ...
              // we'll fallback to the built-in PDF.pdfjs JPEG decoder; see
              // `PartialEvaluator.buildPaintImageXObject` in the
              // `src/core/evaluator.pdfjs` file.
    ...
    messageHandler.on('JpegDecode', function(data) {
      ...
      if (typeof document === 'undefined') {
        // Make sure that this code is not executing in node.pdfjs, as
