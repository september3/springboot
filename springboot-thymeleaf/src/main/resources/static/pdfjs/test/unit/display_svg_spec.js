...
function withZlib(isZlibRequired, callback) {
  if (isZlibRequired) {
    ...
    // For now, only support zlib functionality on Node.pdfjs
    if (!isNodeJS()) {
      throw new Error('zlib test can only be run in Node.pdfjs');
    }
...
describe('SVGGraphics', function () {
  ...
  describe('paintImageXObject', function() {
    ...
    it('should fail require("zlib") unless in Node.pdfjs', function() {
      function testFunc() {
