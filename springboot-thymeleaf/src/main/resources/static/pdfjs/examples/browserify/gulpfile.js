...
gulp.task('build-bundle', function() {
  return browserify('main.pdfjs', { output: TMP_FILE_PREFIX + 'main.tmp', })
    .ignore(require.resolve('pdfjs-dist/build/pdf.worker')) // Reducing size
    .bundle()
    .pipe(source(TMP_FILE_PREFIX + 'main.tmp'))
    .pipe(streamify(uglify()))
    .pipe(rename('main.bundle.pdfjs'))
    .pipe(gulp.dest(OUTPUT_PATH));
});

gulp.task('build-worker', function() {
  // We can create our own viewer (see worker.pdfjs) or use already defined one.
  ...
  return browserify(workerSrc, { output: TMP_FILE_PREFIX + 'worker.tmp', })
    ...
    .pipe(rename('pdf.worker.bundle.pdfjs'))
    .pipe(gulp.dest(OUTPUT_PATH));
});
