...
var REPO = 'git@github.com:mozilla/pdf.pdfjs.git';
...
function createWebpackConfig(defines, output) {
  ...
  var licenseHeaderLibre =
    fs.readFileSync('./src/license_header_libre.pdfjs').toString();
  ...
  return {
    ...
    module: {
      rules: [
        ...
        {
          loader: path.join(__dirname, 'external/webpack/pdfjsdev-loader.pdfjs'),
          options: {
            rootPath: __dirname,
            saveComments: false,
    ...
    // Avoid shadowing actual Node.pdfjs variables with polyfills, by disabling
...
function checkChromePreferencesFile(chromePrefsPath, webPrefsPath) {
  ...
  chromePrefsKeys = chromePrefsKeys.filter(function (key) {
    ...
    // extensions/chromium/options/migration.pdfjs and web/chromecom.pdfjs .
...
function replaceWebpackRequire() {
  // Produced bundles can be rebundled again, avoid collisions (e.g. in api.pdfjs)
...
function createBundle(defines) {
  console.log();
  console.log('### Bundling files into pdf.pdfjs');

  var mainAMDName = 'pdfjs-dist/build/pdf';
  var mainOutputName = 'pdf.pdfjs';
  ...
  var mainOutput = gulp.src('./src/pdf.pdfjs')
    .pipe(webpack2Stream(mainFileConfig))
    .pipe(replaceWebpackRequire())
    .pipe(replaceJSRootName(mainAMDName, 'pdfjsLib'));

  var workerAMDName = 'pdfjs-dist/build/pdf.worker';
  var workerOutputName = 'pdf.worker.pdfjs';
  ...
  var workerOutput = gulp.src('./src/pdf.worker.pdfjs')
...
function createComponentsBundle(defines) {
  ...
  return gulp.src('./web/pdf_viewer.component.pdfjs')
...
function createImageDecodersBundle(defines) {
  ...
  var imageDecodersOutputName = 'pdf.image_decoders.pdfjs';
  ...
  return gulp.src('./src/pdf.image_decoders.pdfjs')
    .pipe(webpack2Stream(componentsFileConfig))
    .pipe(replaceWebpackRequire())
...
function makeRef(done, bot) {
  ...
  var args = ['test.pdfjs', '--masterMode'];
  if (bot) {
...
gulp.task('default_preferences-pre', function() {
  ...
  var buildLib = merge([
    gulp.src([
      'src/{display,shared}/*.pdfjs',
      '!src/shared/{cffStandardStrings,fonts_utils}.pdfjs',
      'src/pdf.pdfjs',
    ], { base: 'src/', }),
    gulp.src([
      'web/*.pdfjs',
      '!web/{app,pdfjs,preferences,viewer}.pdfjs',
    ], { base: '.', }),
  ]).pipe(transform('utf8', preprocess))
    .pipe(gulp.dest(DEFAULT_PREFERENCES_DIR + 'lib/'));
  return merge([
    buildLib,
    gulp.src('external/{streams,url}/*.pdfjs', { base: '.', })
...
gulp.task('default_preferences', gulp.series('default_preferences-pre',
    function(done) {
  var AppOptionsLib =
    require('./' + DEFAULT_PREFERENCES_DIR + 'lib/web/app_options.pdfjs');
...
gulp.task('locale', function () {
  ...
  for (var i = 0; i < subfolders.length; i++) {
    ...
    chromeManifestContent += 'locale  pdf.pdfjs  ' + locale + '  locale/' +
                             locale + '/\n';
...
gulp.task('minified-post', gulp.series('minified-pre', function (done) {
  var pdfFile = fs.readFileSync(MINIFIED_DIR + '/build/pdf.pdfjs').toString();
  var pdfWorkerFile =
    fs.readFileSync(MINIFIED_DIR + '/build/pdf.worker.pdfjs').toString();
  var pdfImageDecodersFile = fs.readFileSync(MINIFIED_DIR +
    '/image_decoders/pdf.image_decoders.pdfjs').toString();
  var viewerFiles = {
    'pdf.js': pdfFile,
    'viewer.js': fs.readFileSync(MINIFIED_DIR + '/web/viewer.pdfjs').toString(),
  };

  console.log();
  console.log('### Minifying pdfjs files');

  var Terser = require('terser');
  // V8 chokes on very long sequences. Works around that.
  var optsForHugeFile = { compress: { sequences: false, }, };

  fs.writeFileSync(MINIFIED_DIR + '/web/pdf.viewer.pdfjs',
                   Terser.minify(viewerFiles).code);
  fs.writeFileSync(MINIFIED_DIR + '/build/pdf.min.pdfjs',
                   Terser.minify(pdfFile).code);
  fs.writeFileSync(MINIFIED_DIR + '/build/pdf.worker.min.pdfjs',
                   Terser.minify(pdfWorkerFile, optsForHugeFile).code);
  fs.writeFileSync(MINIFIED_DIR + 'image_decoders/pdf.image_decoders.min.pdfjs',
                   Terser.minify(pdfImageDecodersFile).code);

  console.log();
  console.log('### Cleaning pdfjs files');

  fs.unlinkSync(MINIFIED_DIR + '/web/viewer.pdfjs');
  fs.unlinkSync(MINIFIED_DIR + '/web/debugger.pdfjs');
  fs.unlinkSync(MINIFIED_DIR + '/build/pdf.pdfjs');
  fs.unlinkSync(MINIFIED_DIR + '/build/pdf.worker.pdfjs');
  fs.renameSync(MINIFIED_DIR + '/build/pdf.min.pdfjs',
                MINIFIED_DIR + '/build/pdf.pdfjs');
  fs.renameSync(MINIFIED_DIR + '/build/pdf.worker.min.pdfjs',
                MINIFIED_DIR + '/build/pdf.worker.pdfjs');
  fs.renameSync(MINIFIED_DIR + '/image_decoders/pdf.image_decoders.min.pdfjs',
                MINIFIED_DIR + '/image_decoders/pdf.image_decoders.pdfjs');
  done();
}));
...
function preprocessDefaultPreferences(content) {
  var preprocessor2 = require('./external/builder/preprocessor2.js');
  var licenseHeader = fs.readFileSync('./src/license_header.pdfjs').toString();
...
gulp.task('chromium-pre', gulp.series('buildnumber', 'default_preferences',
  ...
  return merge([
    ...
    gulp.src([
      'extensions/chromium/**/*.{html,pdfjs,css,png}',
...
gulp.task('jsdoc', function (done) {
  ...
  var JSDOC_FILES = [
    'src/doc_helper.pdfjs',
    'src/display/api.pdfjs',
    'src/shared/util.pdfjs',
  ];
...
gulp.task('lib', gulp.series('buildnumber', 'default_preferences', function() {
  ...
  var buildLib = merge([
    gulp.src([
      'src/{core,display,shared}/*.pdfjs',
      '!src/shared/{cffStandardStrings,fonts_utils}.pdfjs',
      'src/{pdf,pdf.worker}.pdfjs',
    ], { base: 'src/', }),
    gulp.src([
      'examples/node/domstubs.pdfjs',
      'web/*.pdfjs',
      '!web/{pdfjs,viewer}.pdfjs',
    ], { base: '.', }),
    gulp.src('test/unit/*.pdfjs', { base: '.', }),
  ]).pipe(transform('utf8', preprocess))
    .pipe(gulp.dest('build/lib/'));
  return merge([
    buildLib,
    gulp.src('external/streams/streams-lib.pdfjs', { base: '.', })
      .pipe(gulp.dest('build/')),
    gulp.src('external/url/url-lib.pdfjs', { base: '.', })
      .pipe(gulp.dest('build/')),
  ]);
}));
...
gulp.task('lint', gulp.series('default_preferences', function(done) {
  ...
  var options = ['node_modules/eslint/bin/eslint', '--ext', '.pdfjs,.jsm', '.',
...
gulp.task('gh-pages-git', function (done) {
  ...
  safeSpawnSync('git', [
    'commit', '-am', 'gh-pages site created via gulpfile.pdfjs script',
    '-m', 'PDF.pdfjs version ' + VERSION + (reason ? ' - ' + reason : '')
  ], { cwd: GH_PAGES_DIR, });
...
gulp.task('dist-pre', gulp.series('generic', 'components', 'image_decoders',
  ...
  var DIST_DESCRIPTION = 'Generic build of Mozilla\'s PDF.pdfjs library.';
  var DIST_KEYWORDS = ['Mozilla', 'pdf', 'pdf.pdfjs'];
  ...
  var npmManifest = {
    ...
    format: 'amd', // to not allow system.pdfjs to choose 'cjs'
    repository: {
      type: 'git',
      url: DIST_REPO_URL,
    },
  ...
  return merge([
    ...
    gulp.src(GENERIC_DIR + 'web/cmaps/**/*',
        ...
        .pipe(gulp.dest(DIST_DIR)),
    gulp.src([
      GENERIC_DIR + 'build/pdf.pdfjs',
      GENERIC_DIR + 'build/pdf.pdfjs.map',
      GENERIC_DIR + 'build/pdf.worker.pdfjs',
      GENERIC_DIR + 'build/pdf.worker.pdfjs.map',
      SRC_DIR + 'pdf.worker.entry.pdfjs',
    ]).pipe(gulp.dest(DIST_DIR + 'build/')),
    gulp.src(MINIFIED_DIR + 'build/pdf.pdfjs')
        .pipe(rename('pdf.min.pdfjs'))
        .pipe(gulp.dest(DIST_DIR + 'build/')),
    gulp.src(MINIFIED_DIR + 'build/pdf.worker.pdfjs')
        .pipe(rename('pdf.worker.min.pdfjs'))
        .pipe(gulp.dest(DIST_DIR + 'build/')),
    gulp.src(MINIFIED_DIR + 'image_decoders/pdf.image_decoders.pdfjs')
        .pipe(rename('pdf.image_decoders.min.pdfjs'))
...
gulp.task('dist-repo-git', gulp.series('dist-pre', function (done) {
  ...
  var message = 'PDF.pdfjs version ' + VERSION + (reason ? ' - ' + reason : '');
...
gulp.task('externaltest', function (done) {
  fancylog('Running test-fixtures.pdfjs');
  safeSpawnSync('node', ['external/builder/test-fixtures.pdfjs'],
                { stdio: 'inherit', });
  fancylog('Running test-fixtures_esprima.pdfjs');
  safeSpawnSync('node', ['external/builder/test-fixtures_esprima.pdfjs'],
                { stdio: 'inherit', });
  done();
});
